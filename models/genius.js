...
Genius.prototype.getArtistByName = async function (name) {
    let song;
...
Genius.prototype.getLyrics = async function (songUrl) {
    try {
        ...
        return Promise.resolve(cheerio.parseSongHTML(finalResponse));
    } catch (err) {
        if (!err.statusCode !== 500) {
            err.statusCode = 500;
        }
